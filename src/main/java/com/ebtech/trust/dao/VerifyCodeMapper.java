...
public interface VerifyCodeMapper {
  ...
  @Select("select * from `verify_code` where phone = #{phone}")
  VerifyCode selectVerifyCodeByPhone(@Param("phone") String phone);

  @Insert("insert into verify_code(phone, code, create_at, expire) values(#{phone}, #{code}, now(), #{expire})")
  void insertVerifyCode(VerifyCode verifyCode);

  @Update("update verify_code set code = #{code}, create_at = now(), expire = #{expire} where phone = #{phone}")
  void updateVerifyCode(VerifyCode verifyCode);

  @Delete("delete from `verify_code` where phone = #{phone}")
